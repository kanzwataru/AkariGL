...
ShaderID agl_load_compile_shader(const char *vert_path, const char *geom_path, const char *frag_path)
{
    ...
    char *geom_src = agl_filesys_load(geom_path, NULL);

    ShaderID shader = agl_compile_shader(vert_src, geom_src, frag_src);
    free(vert_src);
    free(frag_src);
    if(geom_src)
        free(geom_src);
...
ShaderID agl_compile_shader(const char *vert_src, const char *geom_src, const char *frag_src)
{
    GLuint vert, geom, frag, shader;
